...
def _find_model_path(model_root: Path) -> Path:
    for basename in MODEL_BASENAMES:
        model_path = model_root / basename
        if model_path.is_file():
            return model_path
    raise FileNotFoundError(
        f"Model file not found under {model_root} (expected one of: "
        f"{', '.join(MODEL_BASENAMES)})"
    )
...
def resolve_model_directory() -> Path:
    """Directory containing an accepted model joblib (after optional fetch/extract)."""

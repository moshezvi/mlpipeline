...
def _validated_tar_members(tf: tarfile.TarFile, dest: Path) -> list[TarInfo]:
    """Return tar members that are safe to extract under dest."""
    ...
    for member in tf.getmembers():
        ...
        if member.issym() or member.islnk() or not (member.isfile() or member.isdir()):
            raise ValueError(
                f"Unsupported tar member type in model artifact tarball: {member.name!r}"
            )

...
Supports local directories and fetched MODEL_ARTIFACT_URI values (S3, local file,
or tarball).
"""
...
def _find_model_root(search_root: Path) -> Path:
    ...
    raise FileNotFoundError(
        f"one of {', '.join(MODEL_BASENAMES)} not found under {search_root} "
        "(after extract or download)"
    )


def _validated_tar_members(tf: tarfile.TarFile, dest: Path) -> list[tarfile.TarInfo]:
    dest_root = dest.resolve()
    safe_members = []

    for member in tf.getmembers():
        member_target = (dest / member.name).resolve()
        if os.path.commonpath([dest_root, member_target]) != str(dest_root):
            raise ValueError(f"Unsafe path in model artifact tarball: {member.name}")
        if member.islnk() or member.issym() or member.isdev():
            raise ValueError(
                f"Unsupported tar member in model artifact tarball: {member.name}"
            )
...
def _extract_tarball(archive: Path, dest: Path) -> Path:
    ...
    with tarfile.open(archive, "r:*") as tf:
        tf.extractall(dest, members=_validated_tar_members(tf, dest))
...
def _materialize_uri_to_model_root(uri: str, model_dir: Path) -> Path:
    ...
    if uri.startswith("s3://"):
        ...
        raise FileNotFoundError(
            f"After S3 download, expected one of {', '.join(MODEL_BASENAMES)} "
...
def load_model(artifacts_dir: str):
    ...
    if model_path is None:
        raise FileNotFoundError(
            f"Model file not found under {base}; "
            f"expected one of {', '.join(MODEL_BASENAMES)}"
        )

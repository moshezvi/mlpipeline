...
def _find_model_root(search_root: Path) -> Path:
    ...
    raise FileNotFoundError(
        f"one of {MODEL_BASENAMES} not found under {search_root} "
...
def _extract_tarball(archive: Path, dest: Path) -> Path:
    ...
    with tarfile.open(archive, "r:*") as tf:
        members = list(_validated_tar_members(tf, dest))
...
def _materialize_uri_to_model_root(uri: str, model_dir: Path) -> Path:
    ...
    if uri.startswith("s3://"):
        ...
        raise FileNotFoundError(
            f"After S3 download, expected one of {MODEL_BASENAMES} "
...
def load_model(artifacts_dir: str):
    ...
    if model_path is None:
        raise FileNotFoundError(f"Model file not found under {base}: {MODEL_BASENAMES}")

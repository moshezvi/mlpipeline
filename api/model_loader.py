...
def _find_model_path(search_root: Path) -> Path:
    ...
    raise FileNotFoundError(
        f"None of {MODEL_BASENAMES} found under {search_root} (after extract or download)"
    )


def _ensure_safe_tar_member(member: tarfile.TarInfo, dest: Path) -> None:
    if member.issym() or member.islnk():
        raise ValueError(f"Unsafe tar member link rejected: {member.name}")
...
def _extract_tarball(archive: Path, dest: Path) -> Path:
    ...
    with tarfile.open(archive, "r:*") as tf:
        ...
        for member in members:
            _ensure_safe_tar_member(member, resolved_dest)
